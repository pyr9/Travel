<template>
    <div class="search">
        <input v-model="keyWord" class="search-input" placeholder="请输入城市或拼音" type="text">
        <div v-show="keyWord" ref="search" class="search-content">
            <ul>
                <li v-for="city in list" :key="city.id" class="search-item border-bottom"
                    @click="handleCityClick(city.name)">
                    {{ city.name }}
                </li>
                <li v-show="hasNoData" class="search-item border-bottom">没有出现匹配数据</li>
            </ul>
        </div>
    </div>
</template>
